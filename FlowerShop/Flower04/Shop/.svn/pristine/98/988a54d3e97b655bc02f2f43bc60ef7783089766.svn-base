...
<script type="text/javascript">
	function toPay() {
		//通过class 获取id
		var aid= $(".detail-name").attr("id")
		if(aid==null){
			alert("请选择地址");
			return false;
		}
		
		window.location.href = "/Shop/toPay?id=" + aid;
	}
</script>
</head>
<body>
	...
	<div class="orderDetail row"> 
		...
		<div class="panel panel-success col-md-10 col-md-offset-1 border-detail">
		  ...
		  <tbody>
		  	<c:forEach items="${cartlist}" var="ce">	
		    <tr>
		      		<td><img  href="goodsDetail?id=${ce.id}"  width="60px" height="60px;" src="<%=basePath %>logos/${ce.gpicture}"></td>
		      		<td>${ce.gname}</td>
		   ...
		   <tfoot>
		    <tr>
		      <td  colspan="5">
		      <div class="td-pay">
		      	<span>总计:</span><span class="item-price">￥${total}</span>
		      	<button type="button" class="btn btn-danger" onclick="toPay()">生成订单</button>
		      </div>
