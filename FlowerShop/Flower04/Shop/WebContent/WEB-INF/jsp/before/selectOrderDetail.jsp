...
<body>
	...
	<div class="orderDetail row"> 
		...
		<ol class="breadcrumb col-md-10 col-md-offset-1">
		  <li><a href="<%=basePath %>useC">用户中心</a></li>
		  <li><a href="<%=basePath %>cart/selectCart">购物车</a></li>
		  ...
		  <div class="panel-heading">收货信息</div>
		    <table class="table">
		    	...
		    	<tbody>
		    		...
		    		<tr>
		    			<td class="detail-name">${address.receiver }</td>
		    			<td class="detail-phone">${address.receiverphone  }</td>
		    			<td class="detail-address">${address.address}</td>
		    		</tr>
		    	</tbody>
		    </table>
		...
		<div class="panel panel-success col-md-10 col-md-offset-1 border-detail">
		  ...
		  <span>订单详情</span>
		  <span class="ordertitle">订单号:</span><span class="orderNumber">${address.id }</span>
		  </div>
		  ...
		  <tbody class="no-paid-orderDetail">
			  <c:forEach items="${ goods }" var="sg" varStatus="status">
			    <tr>
			      ...
			      <td><a href="<%=basePath %>/goodsDetail?id=${sg.id}">${sg.gname }</a></td>
			      <td>${sg.shoppingnum }</td>
			      <td>￥${sg.orderprice}</td>
			      <td >￥<span class="goods-ordercount">${sg.ordercount}</span></td>
			    </tr>
				</c:forEach>		  		
		  </tbody>
		 ...
		 <c:if test="${address.status == 0 }">
		  <tfoot>
		    <tr>
		      <td colspan="5">
		      <div class="td-pay">
		      	<span>总计:</span><span class="item-price">￥<span class="sum-paid">8888</span></span>
		      	<a href="/Shop/order/pay/${address.id }"><button type="button" class="btn btn-danger">去支付</button></a>
		      </div> 
		      </td>
		    </tr>
		  </tfoot>
